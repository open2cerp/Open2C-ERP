...
bool CExtPopupMenuWnd::MENUITEMDATA::SetPopupText( LPCTSTR strText )
{
	SetText( strText );
	int nPos = m_sItemText.Find(_T('\t'));
	if( nPos >= 0 )
	{
		CString sLeft = m_sItemText.Left(nPos);
		CString sRight = m_sItemText.Right(m_sItemText.GetLength()-nPos-1);
		m_sItemText = sLeft;
		m_sAccelText = sRight;
	}
	return true;
...
bool CExtPopupMenuWnd::MENUITEMDATA::SetPopupAccelText( LPCTSTR strText ) // NULL if empty
{
	m_sAccelText =
...
LPCTSTR CExtPopupMenuWnd::MENUITEMDATA::GetText() const
{
	if( IsSeparator() )
		return _T("");
	//	if( IsPopup() )
	//		return (LPCTSTR)m_sItemText;
	//	return GetCmd()->m_sMenuText;
	return (LPCTSTR)m_sItemText;
}

void CExtPopupMenuWnd::MENUITEMDATA::SetText( LPCTSTR strText )
{
	if( IsSeparator() )
		return;
...
LPCTSTR CExtPopupMenuWnd::MENUITEMDATA::GetAccelText() const
{
	if( IsSeparator() )
		return _T("");
	//if( IsPopup() )
	return (LPCTSTR)m_sAccelText;
	//return GetCmd()->m_sAccelText;
}

void CExtPopupMenuWnd::MENUITEMDATA::SetAccelText( LPCTSTR strText )
{
	if( IsSeparator() )
		return;
...
int CExtPopupBaseWnd::OnMouseActivate(CWnd* pDesktopWnd, UINT nHitTest, UINT message) 
{
	...
	if( hWndInPlace != NULL )
	{
		...
		VERIFY( ::GetCursorPos(&ptScreenClick) );
...
, m_bHelperAddRemSeparator( false )
...
#if (!defined __EXT_MFC_NO_CUSTOMIZE)
		LPCTSTR strProfileName = g_CmdManager->ProfileNameFromWnd(m_hWndCmdReciever);
									 ...
									 )
{
	...
	if( bHasInplaceItems )
	{
		...
		for( i = 0; i < nCount; i++ )
		{
			...
			// when using inplace editors,
			//all the items should be expanded
			ASSERT( v.GetSize() == nCount );
			
			VisibleItemDefinition_t & vi = v[ i ];
			int nInplaceEditWidth = mi.GetInplaceEditWidth();
			ASSERT( nInplaceEditWidth > 0 );
			CRect rc( vi.m_rcItem );
			rc.DeflateRect( 1, 1, 1, 1+__EXT_MENU_VERT_DISTANCE_NORM );
			rc.left = rc.right - nInplaceEditWidth;
												  ...
												  )
{
	...
	else
	{
		...
		{
			((MENUITEMDATA *)m_pItemData)->GetInplaceEditPtr()->GetWindowRect( &_pmid.m_rcInPlaceWnd );
			m_pPopup->ScreenToClient( &_pmid.m_rcInPlaceWnd );
		} // if( ((MENUITEMDATA *)m_pItemData)->IsInplaceEdit() )
	...
	if( m_pPopup->_FindCustomizeMode() )
	{
		CExtCustomizeSite * pSite = m_pPopup->_FindCustomizeSite();
		ASSERT( pSite != NULL );
		if(		pSite != NULL
			&&	(		(	pSite->DraggedNodeGet() != NULL
			&&	(	pSite->DraggedNodeGet() == ((MENUITEMDATA*)m_pItemData)->GetCmdNode()
			||	( m_pItemData->IsSelected() && m_pItemData->IsPopup() )
			)
			)
								...
								)
{
	...
	{ // paint visible items
		...
		DRAWITEMDATA _DrawItemData(
			...
			this,
			(	(!_FindCustomizeMode())
			&&	( GetSite()->m_dwTrackFlags&TPMX_OWNERDRAW_FIXED ) == 0
			)
			? true : false,
			vi.m_bHelperIsPopup,
			g_bMenuHighlightRarely && vi.m_bRarelyUsed,
	...
	{ // if new popup tracking
		...
		if( mi.GetPopup()->GetSafeHwnd() == NULL )
		{
			bDisplayedSuccessfully = 
				...
				| ( m_dwTrackFlags & TPMX_NO_SHADOWS )
				,
				point.x,point.y,
				&rectItem,
				(LPVOID)m_nCurIndex,
											 ...
											 )
{
	...
	ASSERT( nItemCount == 0 || m_nHelperDropIndexBefore <= nItemCount );
	...
	if( nItemCount > 0 )
	{
		if(		m_nHelperDropIndexBefore == 0
			&&	_GetItemRef(m_nHelperDropIndexBefore).IsSeparator()
			)
		{
			ASSERT( FALSE );
			_SetDropIndexBefore();
		...
		if(		de == DROPEFFECT_MOVE
			&&	((LPVOID)_dragInfo.m_pDropNodeC->GetParentNode()) == ((LPVOID)m_pNode)
			&&	m_nHelperDropIndexBefore < nItemCount
			)
		{
			MENUITEMDATA & miDropPreTest = _GetItemRef(m_nHelperDropIndexBefore);
			CExtCustomizeCmdTreeNode * pNodeOver = NULL;
			if( !miDropPreTest.IsSeparator() )
			{
				pNodeOver = miDropPreTest.GetCmdNode();
				ASSERT_VALID( pNodeOver );
			} // if( !miDropPreTest.IsSeparator() )
			else
			{
				if( m_nHelperDropIndexBefore > 0 )
				{
					ASSERT( !_GetItemRef(m_nHelperDropIndexBefore-1).IsSeparator() );
					pNodeOver = _GetItemRef(m_nHelperDropIndexBefore-1).GetCmdNode();
					ASSERT_VALID( pNodeOver );
				} // if( m_nHelperDropIndexBefore > 0 )
			} // else from if( !miDropPreTest.IsSeparator() )
			
			if( ((LPVOID)pNodeOver) == ((LPVOID)_dragInfo.m_pDropNodeC)  )
			{
				if( pNodeOver->GetFlags() & __ECTN_GROUP_START )
		...
		if(		m_nHelperDropIndexBefore > 0
			&&	m_nHelperDropIndexBefore < nItemCount
			&&	_GetItemRef( m_nHelperDropIndexBefore - 1 ).IsSeparator()
			)
			bPrevSeparator = true;
	} // if( nItemCount > 0 )
	else
	...
	bool bRetVal = true;
	m_bHelperNoHideChild = true;
	if( bSeparatorRemove || bSeparatorAdd )
	{
		ASSERT( m_nHelperDropIndexBefore < nItemCount );
		bRetVal = false;
		m_bHelperAddRemSeparator = true;
		if( abs(point.y-m_ptHelperDragStart.y) <= 3 )
		{
			_SetDropIndexBefore();
			m_ptHelperDragStart.x = m_ptHelperDragStart.y = 0;
			return bRetVal;
		...
		if( bSeparatorAdd )
		{
			ASSERT( !bSeparatorRemove );
			_dragInfo.m_pDropNodeC->ModifyFlags( __ECTN_GROUP_START );
			ASSERT( m_nHelperDropIndexBefore > 0 );
			MENUITEMDATA mi;
			mi.SetCmdReciever( m_hWndCmdReciever );
			mi.SetSeparator( m_nHelperDropIndexBefore );
			ASSERT( mi.IsSeparator() );
		...
		{
			ASSERT( bSeparatorRemove );
			ASSERT( m_nHelperDropIndexBefore > 1 );
			_dragInfo.m_pDropNodeC->ModifyFlags( 0, __ECTN_GROUP_START );
			ASSERT( _GetItemRef(m_nHelperDropIndexBefore-1).IsSeparator() );
			m_items_all.RemoveAt( m_nHelperDropIndexBefore-1, 1 );
			m_nCurIndex = -1;
		} // else from if( bSeparatorAdd )
	} // if( bSeparatorRemove || bSeparatorAdd )
	else
	{ // if command drop
		m_bHelperNoHideChild = false;
		_ItemFocusCancel( TRUE );
		CExtCustomizeCmdTreeNode * pNode =
			_dragInfo.m_pDropNodeC->CloneNode( pSite );
		ASSERT_VALID( pNode );
		int nNodeInsertPos = -1;
		if( nItemCount > 0 && m_nHelperDropIndexBefore < nItemCount )
		{
			MENUITEMDATA & miDropPos = _GetItemRef( m_nHelperDropIndexBefore );
			if( miDropPos.IsSeparator() )
				nNodeInsertPos =
				_GetItemRef( m_nHelperDropIndexBefore + 1 ).GetCmdNode()
				-> GetOwnIndex();
			else
				nNodeInsertPos = miDropPos.GetCmdNode()->GetOwnIndex();
		} // if( nItemCount > 0 && m_nHelperDropIndexBefore < nItemCount )
		if( bPrevSeparator )
		{
			ASSERT( m_nHelperDropIndexBefore < nItemCount );
			ASSERT( !_GetItemRef( m_nHelperDropIndexBefore ).IsSeparator() );
			pNode->ModifyFlags( __ECTN_GROUP_START );
			_GetItemRef( m_nHelperDropIndexBefore ).GetCmdNode()
				-> ModifyFlags( 0, __ECTN_GROUP_START );
		} // if( bPrevSeparator )
		else
		...
		if( _dragInfo.m_pDropNodeI != NULL )
		{
			...
			pNode->SetCmdID(
				_dragInfo.m_pDropNodeI->GetCmdID( false ),
				false
				);
			pNode->SetCmdID(
				_dragInfo.m_pDropNodeC->GetCmdID( true ),
				true
				);
		} // if( _dragInfo.m_pDropNodeI != NULL )
		m_pNode->InsertNode(
		...
		ASSERT( !mi.IsSeparator() );
		ASSERT( ((LPVOID)mi.GetCmdNode()) == ((LPVOID)pNode) );
		if( mi.IsPopup() )
		{
			mi.GetPopup()->m_hWndCmdReciever = m_hWndCmdReciever;
			mi.GetPopup()->UpdateFromCmdTree(
													  ...
													  )
{
	...
	if( message == WM_KEYDOWN )
	{
		...
		{
			ASSERT( m_pStr != NULL );
			GetWindowText( *m_pStr );
			GetPopupMenu()->SetFocus();
			return 0;
		}
		bool bAlt =
			( (::GetAsyncKeyState(VK_MENU)&0x8000) != 0 )
