...
BEGIN_MESSAGE_MAP(CCrystalEditView, CCrystalTextView)
	...
	ON_WM_CHAR()
	ON_COMMAND(ID_EDIT_DELETE_STRING, OnDeleteString)
	ON_COMMAND(ID_EDIT_DELETE_WORD_BACK, OnDeleteWordBack)
	...
	ON_COMMAND(ID_FORMAT_BLOCK, OnFormatBlock)
	ON_WM_LBUTTONDBLCLK()
	ON_COMMAND(ID_LOWER_WORD, OnLowerWord)
	ON_COMMAND(ID_UPPER_WORD, OnUpperWord)
	//}}AFX_MSG_MAP
...
//Ю.Н.И.
void CCrystalEditView::OnDeleteString() 
{
	if (! QueryEditable() || m_pTextBuffer == NULL)
		return;

	CPoint ptSelStart=GetCursorPos();
	if(ptSelStart.y>=0 && ptSelStart.y<m_pTextBuffer->GetLineCount())
	{
		int x2=0;
		int y2=ptSelStart.y+1;
		if(y2>=m_pTextBuffer->GetLineCount())
		{
			x2=m_pTextBuffer->GetLineLength(ptSelStart.y);
			y2=ptSelStart.y;
		}
		m_pTextBuffer->DeleteText(this, ptSelStart.y, 0, y2, x2, CE_ACTION_DELETE); // [JRT]
	}
}

//Ю.Н.И.
void CCrystalEditView::OnDeleteWordBack() 
{
	MoveWordLeft(1);
	OnEditDelete();
}
...
void CCrystalEditView::OnFormatBlock() 
{
	FormatSelection(0);	
}


void CCrystalEditView::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
...
void CCrystalEditView::OnLowerWord() 
{
	SetLowerWord(1);
}

void CCrystalEditView::OnUpperWord() 
{
	SetLowerWord(0);
}
