...
BOOL CCrystalTextView::IsLetter(char pszChars0)
{
	unsigned int pszChars=pszChars0;
	if((pszChars >= 'А' && pszChars <= 'Я')||(pszChars >= 'а' && pszChars <= 'я')||
		(pszChars >= 'A' && pszChars <= 'Z')|(pszChars >= 'a' && pszChars <= 'z')||
		(pszChars >= '0' && pszChars <= '9')||(pszChars == '_'))//||(pszChars == '.'))
			return TRUE;
	return FALSE;
}

BOOL CCrystalTextView::IsLetter(char pszChars,BOOL flSpacesOnly)
{
	if(!flSpacesOnly)
	{
		return IsLetter(pszChars);
	}
	else
	{
...
{
	if(m_ptCursorPos.y==ptTextPos.y)
		if(ptTextPos.x<m_ptCursorPos.x)
			nCursorColor=nColorIndex;
...
void CCrystalTextView::ResetView()
{
	...
	nCursorColor=COLORINDEX_WHITESPACE;
}
...
int CCrystalTextView::CalculateActualOffset(int nLineIndex, int nCharIndex)
{
	...
	int nLength = GetLineLength(nLineIndex);
	if(nCharIndex >= 0 && nCharIndex <= nLength)
	{
		LPCTSTR pszChars = GetLineChars(nLineIndex);
		int nOffset = 0;
		int nTabSize = GetTabSize();
...
int CCrystalTextView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	lstrcpy(m_lfBaseFont.lfFaceName, _T("Courier"));
	m_lfBaseFont.lfHeight = 0;
	m_lfBaseFont.lfWeight = FW_THIN;
...
static int FindStringHelper(LPCTSTR pszFindWhere, LPCTSTR pszFindWhat, BOOL bWholeWord)
{
	...
	for (;;)
	{
		...
		if (pszPos > pszFindWhere && (CCrystalTextView::IsLetter(pszPos[-1]) || pszPos[-1] == _T('_')))
		{
			nCur += (pszPos - pszFindWhere);
			pszFindWhere = pszPos + 1;
			continue;
		}
		if (CCrystalTextView::IsLetter(pszPos[nLength]) || pszPos[nLength] == _T('_'))
		{
...
void CCrystalTextView::OnToggleBookmark()
{
	if (m_pTextBuffer != NULL)
	{
		...
		m_pTextBuffer->SetLineFlag(m_ptCursorPos.y, dwMask, ((dwFlags & dwMask) == 0)||(dwFlags &LF_BREAKPOINT), FALSE);
	}
...
{
	Str.MakeUpper();
	int n=(int)aHashKeywordList[Str];
	return n;
}

#define DEFINE_BLOCK(colorindex)	\
{\
	ASSERT((nStartPos) >= 0 && (nStartPos) <= nLength);\
	if (pBuf != NULL)\
	{\
		if (nActualItems == 0 || pBuf[nActualItems - 1].m_nCharPos <= (nStartPos)){\
		pBuf[nActualItems].m_nCharPos = (nStartPos);\
...
//	return dwCookie;
	...
	char *p=(char*)pszChars;
	char c=p[nLength];
	p[nLength]=0;
	translate.Load(pszChars);
	p[nLength]=c;

	CString s;
	translate.nPos=0;
	int nStartPos=translate.nPos;

	CString Str=pszChars;
	Str.TrimLeft();
	if(Str.Left(1)=="|")
	{
		DEFINE_BLOCK(COLORINDEX_STRING);
		return dwCookie;
	}

	//вдруг строка начинается с комментария:
	DEFINE_BLOCK(COLORINDEX_COMMENT);

	bool bWasLeftPoint=0;
	while(!translate.IsEnd())
	{
		nStartPos=translate.nPos;//запоминаем позицию цветовыделения текста
		if(translate.IsWord())
		{
			s=translate.GetWord();
			int n=IsKeyWord(s);
			if(n>0&&bWasLeftPoint==0)
			{
				if(s.Left(1)=="#")
				{
					DEFINE_BLOCK(COLORINDEX_PREPROCESSOR)
				}
				else
				{
					DEFINE_BLOCK(COLORINDEX_KEYWORD)
				}
			}
			else
			{
				DEFINE_BLOCK(COLORINDEX_NORMALTEXT)
			}
			bWasLeftPoint=0;
		}
		else
		if(translate.IsNumber()||translate.IsString()||translate.IsDate())
		{
...
/*
	...
	for (int I = 0; ; I++)
	{
		...
		if (pszChars[I] == ' ' )
			if( (pszChars[I+1] >= 'А' && pszChars[I+1] <= 'Я') ||(pszChars[I+1] >= 'а' && pszChars[I+1] <= 'я'))
		...
		if (IsLetter(pszChars[I]) || pszChars[I] == '_' || pszChars[I] == '.'||IsLetter(pszChars[I]))
		{
			if (nIdentBegin == -1)
				nIdentBegin = I;
