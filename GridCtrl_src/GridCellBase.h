...
typedef struct _GV_ITEM {
    ...
    int      iImage;      // index of the list view itemТs icon
...
//    virtual void SetLeftSpace(int nCount)        	    { ASSERT( FALSE);    }
    virtual void SetText(LPCTSTR /* szText */)              { ASSERT( FALSE);    }
    virtual void SetValue(CValue &/* szText */)              { ASSERT( FALSE);    }
    virtual void SetData(CValue &Val)						{ ASSERT( FALSE);    }
	...
	virtual void SetCovered(int nCoveredRow,int nCoveredCol){ ASSERT( FALSE);    }
	virtual void SetCoveredCell(CGridCellBase*){ASSERT(FALSE);};
	virtual void SetCell(class CCell *){ASSERT( FALSE);};

//    virtual int  GetLeftSpace(void) const	        	    { ASSERT( FALSE);return 0;}
    virtual LPCTSTR  GetText() const                        { ASSERT( FALSE); return NULL;  }
    virtual CValue   GetValue()const						{ ASSERT( FALSE); return CValue();}
    virtual CValue*  GetPValue()const						{ ASSERT( FALSE); return NULL;}
	...
	virtual BOOL IsCovered(){ ASSERT( FALSE); return FALSE;   }
	//virtual void GetCovered(int &nCoveredRow,int &nCoveredCol){ ASSERT(FALSE);}
	virtual int GetCoveredRow(){return 0;};//кол-во перекрываемых строк
	virtual int GetCoveredCol(){return 0;};//кол-во перекрываемых колонок
	virtual CGridCellBase *GetCoveredCell(){return 0;};//или ссылка на перекрываемую €чейку
	virtual class CCell *GetCell()const{ASSERT( FALSE); return 0;};
	...
	BOOL GetCoveredRowCol(int &row,int &col);
